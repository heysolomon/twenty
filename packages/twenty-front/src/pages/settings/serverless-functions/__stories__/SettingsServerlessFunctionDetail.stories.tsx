...
export const Default: Story = {
  play: async ({ canvasElement }) => {
    ...
    const testTab = await canvas.findByTestId('tab-test');
    await userEvent.click(testTab);
    await sleep(100);
    await canvas.findByText('Test your function');

    const settingsTab = await canvas.findByTestId('tab-settings');
    await userEvent.click(settingsTab);
    await sleep(100);
    await canvas.findByText('About');

    const editorTab = await canvas.findByTestId('tab-editor');
